...
$path->close_path;
my $line = $path->get_primitive($path->count_primitives - 1);
isa_ok($line, 'Geometry::Primitive::Line');
$chkpt->x(0); $chkpt->y(0);
ok($line->point_end->equal_to($chkpt), 'close path');

$path->arc(5, 0, 1.7);
my $a_line = $path->get_primitive($path->count_primitives - 2);
my $arc = $path->get_primitive($path->count_primitives - 1);
